...
function geneticSolver(
  ...
  for (let round = 0; round < forRounds; round++) {
    ...
    if (withGroupLeaders) {
	  ...
	  console.log("Round", round + 1, "groups:", JSON.stringify(rounds[round], null, 2));
...
/**
 * Given the player groupings produced by geneticSolver, assigns physical table numbers
 * to each group in each round to minimize repeated table assignments per player.
 * Uses the same squared-penalty scoring as the genetic solver.
 * Exhaustive permutation search — practical for ≤7 tables (7! = 5040 perms/round).
 *
 * @param {number[][][]} rounds - rounds[round][group] = [playerIndex, ...]
 * @param {number} numPlayers - total number of players
 * @returns {number[][]} tableAssignments[round][group] = physicalTableIndex (0-based)
 */
function tableRotationSolver(rounds, numPlayers) {
  if (rounds.length === 0) return [];
  const numTables = rounds[0].length;

  // tableVisits[player][table] = how many times player has sat at that physical table
  const tableVisits = Array.from({ length: numPlayers }, () => new Array(numTables).fill(0));

  const assignments = [];
  const scores = [];

  for (const round of rounds) {
    const perms = generatePermutations(_.range(numTables));

    let bestScore = Infinity;
    let bestPerm = _.range(numTables);

    for (const perm of perms) {
      let score = 0;
      for (let g = 0; g < numTables; g++) {
        const t = perm[g];
        for (const player of round[g]) {
          score += Math.pow(tableVisits[player][t], 2);
        }
      }
      if (score < bestScore) {
        bestScore = score;
    ...
    // Update visits for the next round's scoring
    for (let g = 0; g < numTables; g++) {
      const t = bestPerm[g];
      for (const player of round[g]) {
        tableVisits[player][t]++;
      }
    }

    assignments.push(bestPerm);
